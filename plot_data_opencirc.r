...
plot_data_opencirc <- function(filename,plot_save_pdf,plot_save_png,mix,source,discr){
  if(mix$n.iso==1){
...
# -------------------------------------------------------------------------
# This is the code used to make the mantis shrimp isospace
# -------------------------------------------------------------------------      
      print(ggplot(data = df,aes(x = x,y = y),environment=.e) +
            geom_point(aes(shape = factor(mix$FAC[[1]]$values)), show_guide=F) +  # Factor.1
