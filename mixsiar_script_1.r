...
mix <- load_mix_data(filename="mantis_consumer.csv", iso_names=c("d13C","d15N"), random_effects=NULL, cont_effects=NULL, fixed_effects=NULL)
...
source <- load_source_data(filename="mantis_source.csv", source_factors=NULL, conc_dep=FALSE, data_type="means", mix)    
...
jags.1 <- run_model(run="long", indiv_effect,mix,source,discr,model_filename, alpha.prior = alpha.hard) # informative prior
