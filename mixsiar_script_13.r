...
# Mantis shrimp diet analysis #13
#  prior: prey abundance (seagrass)
#  data:  seagrass mantis, seagrass sources
#  error: MixSIR
#  source data from seagrass
...
source <- load_source_data(filename="mantis_source_seagrass.csv", source_factors=NULL, conc_dep=TRUE, data_type="means", mix)    
...
run <- list(chainLength=2000000, burn=1000000, thin=300, chains=3, calcDIC=TRUE)

# Make priors
# AW, BS, Clam, Crab + hermit crab, Fish, Snail
...
alpha.abund <- c(0.37,1.67,0.46,(53.51+0.28),0.29,43.4)*6/100

# Run model
jags.1 <- run_model(run=run, indiv_effect,mix,source,discr,model_filename, alpha.prior = alpha.abund) # informative prior
