...
# Mantis shrimp diet analysis #15
#  prior: "uninformative" (alpha = 1)
#  data:  Habitat + source(Habitat) + concentration dependence
#  error: MixSIR
...
source <- load_source_data(filename="mantis_source_habitat_conc.csv", source_factors="Habitat", conc_dep=TRUE, data_type="means", mix)    
...
# "test" 2 seconds
# "long" didn't time but less than 30 min
jags.1 <- run_model(run="long", indiv_effect,mix,source,discr,model_filename) # UNinformative prior
# jags.1 <- run_model(run="test", indiv_effect,mix,source,discr,model_filename) # UNinformative prior
